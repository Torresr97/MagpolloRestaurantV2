...
namespace AppTRchicken
{
    ...
    public class CrearTicket
    {
        ...
        public void EncabezadoVenta()
        {
            ...
            linea.AppendLine("DESCRIPCION                    CANT TOTAL");
        }
        public void EncabezadoReporteinventario()
        {
            //Escribimos los espacios para mostrar el articulo. En total tienen que ser 40 caracteres
        ...
        public void Agregarinventario(string Producto, int cant)
        {
            ...
            int longitudDeseada = 35;
            string textoFormateado = "";

            while (!string.IsNullOrEmpty(Producto))
            {
                int longitudActual = Math.Min(Producto.Length, longitudDeseada);

                // Obtener la porción del producto que se ajusta a la longitud deseada.
                string parteProducto = Producto.Substring(0, longitudActual);


                if (Producto.Length > longitudDeseada)
                {
                    textoFormateado = parteProducto.PadRight(longitudDeseada);
                    linea.AppendLine(textoFormateado);

                    // Eliminar la porción del producto que ya se ha procesado.
                    Producto = Producto.Substring(longitudActual).TrimStart();
                }
                else
                {
                    // Agregar espacios necesarios al final de la porción del producto y antes del número entero.
                    textoFormateado = parteProducto.PadRight(longitudDeseada) + "  " + cant;
        ...
        public void Encabezadocierre()
        {
            //Escribimos los espacios para mostrar el articulo. En total tienen que ser 40 caracteres
            linea.AppendLine("Desgloce de Billetes           CANT TOTAL");
        }
        ...
        public void TextoIzquierda(string texto)
        {
            ...
            if (texto.Length > 40)
            {
                ...
                for (int longitudTexto = texto.Length; longitudTexto > 40; longitudTexto -= 40)
                {
                    //Agregamos los fragmentos que salgan del texto
                    linea.AppendLine(texto.Substring(caracterActual, 40));
                    caracterActual += 40;
                }
        ...
        public void AgregaArticulo(int cant, string articulo, decimal total)
        {
            ...
            if (cant.ToString().Length <= 5 && total.ToString().Length <= 8)
            {
                ...
                if (articulo.Length > 30)
                {
                    ...
                    elemento += espacios + total.ToString();//Agregamos el precio a la variable elemento
                ...
                {
                    for (int i = 0; i < (30 - articulo.Length); i++)
                    {
        ...
        public void AgregaArticuloscierre(int cant, string articulo, int total)
        {
            //Valida que cant precio e importe esten dentro del rango.
            if (cant.ToString().Length <= 5 && total.ToString().Length <= 8)
            {
