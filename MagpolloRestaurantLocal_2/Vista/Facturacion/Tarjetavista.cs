...
namespace AppTRchicken.Vista
{
    public partial class Tarjetavista : Form
    {
        ...
        private void btncobrar_Click(object sender, EventArgs e)
        {
            ...
            decimal dinerorecibido = Globales.ConvertToDecimal(txtdinerorecibidotarjeta.Text);
        ...
        internal static Tarjetavista Abrir1vez()
        {
            if (((fmrTarjetavista == null) || (fmrTarjetavista.IsDisposed == true)))
            {
                ...
                fmrTarjetavista.Deactivate += fmrTarjetavista_Deactivate;
                fmrTarjetavista.LostFocus += fmrTarjetavista_LostFocus;
                fmrTarjetavista.Resize += fmrTarjetavista_Resize;
            }
            fmrTarjetavista.BringToFront();
            return fmrTarjetavista;
        }
        // Evento cuando el formulario pierde el foco
        private static void fmrTarjetavista_LostFocus(object sender, EventArgs e)
        {
            if (fmrTarjetavista != null && !fmrTarjetavista.Focused)
            {
                // Verificar si el formulario está minimizado o no
                if (fmrTarjetavista.WindowState != FormWindowState.Minimized)
                {
                    fmrTarjetavista.Close();
                }
            }
        }

        // Evento cuando el formulario se minimiza
        private static void fmrTarjetavista_Resize(object sender, EventArgs e)
        {
            if (fmrTarjetavista.WindowState == FormWindowState.Minimized)
            {
                fmrTarjetavista.Close();
        ...
        private static void fmrTarjetavista_Deactivate(object sender, EventArgs e)
        {
            if (fmrTarjetavista != null && !fmrTarjetavista.Focused)
            {
                // Verificar si el formulario está minimizado o no
        ...
        private void txtdinerorecibidotarjeta_TextChanged(object sender, EventArgs e)
        {
            ...
            decimal dinerorecibidoTarjetaDecimal = Globales.ConvertToDecimal(txtdinerorecibidotarjeta.Text);
            decimal ventatotalDecimal = Globales.ConvertToDecimal(txtventatotal.Text);

            // Actualizar Globales.Dinerorecibido
            Globales.Dinerorecibido = dinerorecibidoTarjetaDecimal;

            // Manejar el color de fondo del campo de texto
            if (dinerorecibidoTarjetaDecimal != ventatotalDecimal)
            {
            ...
            if (string.IsNullOrWhiteSpace(txtdinerorecibidotarjeta.Text))
            {
